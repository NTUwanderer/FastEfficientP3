...
// Implements the Population_DSMGA2 structure, which includes pairwise entropy
...
vector<vector<float>> Population_DSMGA2::distances;

Population_DSMGA2::Population_DSMGA2(Configuration& config, size_t _level) {
...
float Population_DSMGA2::get_distance(const array<int, 4>& entry) const {
  array<int, 4> bits;
...
float Population_DSMGA2::get_distance(int x, int y) const {
  if (x > y) {
    std::swap(x, y);
  }
...
void Population_DSMGA2::rebuild_tree(Random& rand) {
...
// of the Parameter-less Population_DSMGA2 Pyramid". Turned
// off by default
bool Population_DSMGA2::k_modeled() {
...
void Population_DSMGA2::improve(Random& rand, vector<bool> & solution, float & fitness,
  ...
  for (auto& cluster_index : cluster_ordering) {
    auto cluster = clusters[cluster_index];
    // Mark all individuals in the Population_DSMGA2 as unused
...
void Population_DSMGA2::smallest_first(Random& rand,
...
void Population_DSMGA2::least_linked_first(Random& rand,
                                    const vector<vector<int>>& clusters,
                                    vector<int>& cluster_ordering) {
...
void Population_DSMGA2::no_action(Random& rand, const vector<vector<int>>& clusters,
                           vector<int>& cluster_ordering) {
}

// Put the clusters in a completely random order
void Population_DSMGA2::random(Random& rand, const vector<vector<int>>& clusters,
                        vector<int>& cluster_ordering) {
  std::shuffle(cluster_ordering.begin(), cluster_ordering.end(), rand);
}
